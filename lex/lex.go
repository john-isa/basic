...
type Lexer struct {
	...
	list         token.TokenList // The list of tokens that represents the program.
...
func (l *Lexer) MakeTokens() token.TokenList {
	for l.current_char != "" {
		...
		case constant.DIGITS:
			l.list = append(l.list, l.make_number())
		case "+":
			l.list = append(l.list, token.New(constant.TT_PLUS, ""))
			l.Advance()
		case "-":
			l.list = append(l.list, token.New(constant.TT_MINUS, ""))
			l.Advance()
		case "/":
			l.list = append(l.list, token.New(constant.TT_DIV, ""))
			l.Advance()
		case "*":
			l.list = append(l.list, token.New(constant.TT_MUL, ""))
			l.Advance()
		case "(":
			l.list = append(l.list, token.New(constant.TT_MUL, ""))
			l.Advance()
		case ")":
			l.list = append(l.list, token.New(constant.TT_RPAREN, ""))
			l.Advance()
		}
	}
	return l.list
